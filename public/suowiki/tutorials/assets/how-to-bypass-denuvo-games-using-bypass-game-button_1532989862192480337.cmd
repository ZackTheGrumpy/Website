...
@::In this case, DSE is temporarily disabled so our custom test-signed driver can be loaded and executed without being rejected by Windows. Without disabling DSE, Windows would prevent the driver from starting and return a digital signature verification error.
@::If Secure Boot is disabled, the script will enable the TESTSIGNING boot configuration option instead of disabling DSE, allowing test-signed code to load.
...
@::A warning will also be shown if Smart App Control, Bitdefender, or MacType are detected, as they may cause compatibility issues.
...
echo.
"%SysPath%\WindowsPowerShell\v1.0\powershell.exe" -nop -c "start cmd.exe -arg ('/c \"\"' + $env:_cmdf + '\" ' + $env:_arg + '\"') -verb runas" >nul 2>&1 && exit /b
)
)
if errorlevel 1 (
cls
...
for /f "delims=" %%a in ('%psc% "if ($PSVersionTable.PSEdition -ne 'Core') {$f=[IO.File]::ReadAllText($env:_cmdf) -split ':pstst';. ([scriptblock]::Create($f[1]))}" 2^>nul') do (set tstresult=%%a)
...
echo  This is a normal Windows feature commonly used when working with test-signed code.
) else (
echo.
...
if defined winhello if defined whcredential (
    ...
    echo.
    echo %cRedHL%If the option to disable Windows Hello is greyed out, either set a password for your account,%cReset%
    echo %cRedHL%or go to Settings ^> Accounts ^> Your info and select "Sign in with a local account", then try again.%cReset%
    echo.
    echo %cYellow%Press any key to exit...%cReset%
    pause >nul
    exit /b
...
sc query MacType | findstr /i "RUNNING" >nul 2>&1
if !errorlevel!==0 (
    sc stop MacType >nul 2>&1
    sc config MacType start= disabled >nul 2>&1
    echo.
    echo Checking MacType                        %cYellow%[Found]%cReset%
    reg add "HKLM\SOFTWARE\ManageVBS" /v MacType /t REG_DWORD /d 1 /f >nul 2>&1
    if "!errorlevel!"=="0" (
        echo Disabling MacType                       %cGreen%[Successful]%cReset%
    ) else (
        echo Disabling MacType                       %cRedHL%[Failed]%cReset%
        reg delete "HKLM\SOFTWARE\ManageVBS" /v MacType /f >nul 2>&1
        set "haderror=1"
    )
)
...
if defined hypneeded (
    ...
    if not defined hypfailed (
        ...
        echo Disabling Windows Hypervisor            %cRedHL%[Failed]%cReset%
...
if /i "!vsmbcd!"=="Auto" (
    ...
    if not defined vsmfailed (
        ...
        echo Disabling Virtual Secure Mode           %cRedHL%[Failed]%cReset%
...
if defined sacstate (
    if "!sacstate!"=="0x1" (
        ...
        echo.
        echo %cGreyHL%Smart App Control may interfere with certain files or components used by the hypervisor.%cReset%
    ...
    if "!sacstate!"=="0x2" (
        ...
        echo.
        echo %cGreyHL%Smart App Control may enable itself after the evaluation period%cReset%
        echo %cGreyHL%and interfere with certain files or components used by the hypervisor.%cReset%
        echo %cGreyHL%It is recommended to disable it in Windows Security.%cReset%
    )
)

for /f "delims=" %%A in ('%psc% "(Get-CimInstance -Namespace root\SecurityCenter2 -Class AntiVirusProduct | Where-Object displayName -eq 'Bitdefender Antivirus').displayName"') do set "bitdefender=%%A"
if defined bitdefender (
    reg query "HKLM\SOFTWARE\Bitdefender" /v atc_data 2>nul | findstr /r "monitor_state.:true" >nul
    if !errorlevel! EQU 0 (
    echo.
    echo Checking Advanced Threat Defense        %cYellow%[Enabled]%cReset%
    echo.
    echo %cBlueHL%Bitdefender's Advanced Threat Defense may cause an "Initialization error 5" message%cReset%
    echo %cBlueHL%in certain games that use VMProtect ^(e.g. Ubisoft and Capcom titles^).%cReset%
    echo.
    echo %cBlueHL%If you encounter this error, open Bitdefender ^> Protection ^> Advanced Threat Defense,%cReset%
    echo %cBlueHL%click "Open", then go to the "Settings" tab and toggle off Advanced Threat Defense.%cReset%
    echo.
    echo %cBlueHL%Alternatively, you can add the game folder as an exception in Bitdefender%cReset%
    echo %cBlueHL%by going to Protection ^> Antivirus ^> Open ^> Settings ^> Manage Exceptions,%cReset%
    echo %cBlueHL%then under All Exceptions, click Add an Exception, select the game folder, and save.%cReset%
    )
)

%psc% "if ((Get-Process explorer).Modules.ModuleName -like '*MacType*'){exit 3}"
if !errorlevel!==3 (
    echo.
    echo Checking MacType                        %cYellow%[Enabled]%cReset%
    echo.
    echo %cBlueHL%MacType may cause certain games to crash during startup.%cReset%
    echo %cBlueHL%You may need to exit MacType from the system tray or uninstall it.%cReset%
    )
)
...
if "!revert_faceit!"=="0x1" (
	...
	sc start FACEIT >nul 2>&1
    sc start FACEITService >nul 2>&1
    if "!errorlevel!"=="0" (
        echo.
...
set "revert_mactype="
for /f "tokens=3" %%A in ('reg query "HKLM\SOFTWARE\ManageVBS" /v MacType 2^>nul') do set "revert_mactype=%%A"
if "!revert_mactype!"=="0x1" (
    sc config MacType start= auto >nul 2>&1
	sc start MacType >nul 2>&1
    if "!errorlevel!"=="0" (
        echo.
        echo Enabling MacType                        %cGreen%[Successful]%cReset%
        reg delete "HKLM\SOFTWARE\ManageVBS" /v MacType /f >nul 2>&1
    ) else (
        echo.
        echo Enabling MacType                        %cRedHL%[Failed]%cReset%
        set "haderror=1"
    )
)
...
if not "!revert_vsm!"=="" (
    ...
    if "!errorlevel!"=="0" (
        ...
        echo.
        echo Enabling Virtual Secure Mode            %cRedHL%[Failed]%cReset%
        set "haderror=1"
    )
)
